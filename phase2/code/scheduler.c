...
// The time at which the last process gonna arrive, used to terminate the
// scheduler after the set of input processes are finished
unsigned int last = -1;
...
struct PCB no_pcb = {NOTSTARTED, {-1, 0, 0, 0, 0}, -1, 0, 0, -1};
// Exits gracefully
void exxit(int);
...
// Allocates memory space for some process using the selected memory allocation
// policy
unsigned int allc();
// Perf logger
void perf();
...
char next_rr(int *pcb_cur) {
  ...
  // THIS WON'T CAUSE AN INF LOOP, YOU HAVE ALREADY CHECKED FOR PCB COUNT, THAT
  // MEANS THERE IS AT LEAST ONE PROCESS IN THE TABLE AND IF ALL PROCESSES CAN'T
  // BE ALLOCATED, THE RUNNING PROCESS WILL GET ALLOCATED
  while (true) {
    (*pcb_cur)++;
    (*pcb_cur) %= MAX_PROC_TABLE_SIZE;
...
void exxit(int signum) {
  ...
  // This is no critical thing, but a non-zero exit code is highlighted in my
  // terminal with a red (x) letter, and it's annoying
  destroyClk(true);
  exit(0);
}
...
void create_new_pcb(struct process *new_p, short is_rr, short is_bud) {
  ...
  if (memo > MAX_MEM_SIZE / 4) {
    // Don't put the process in the proc_table if it needes too much memory,
    // more than 256
    return;
  }
  ...
  if (is_rr == 0) {
    ...
    pcb_ind--;
    pcb_ind %= MAX_PROC_TABLE_SIZE;
    // If it's not free, shift from pcb_curr to the nearest free slot by 1, so
    // to free pcb_curr and put the new process in it
  ...
  pcb[pcb_ind] = (struct PCB){
      NOTSTARTED,
      {
          new_p->_id, new_p->arv, new_p->run, new_p->pri,
          memo ? memo
               : 1 // If a process takes no memory, force it to take at least
                   // one block so it's code segment can be stored somewhere
      },
      -1,
      0,
      new_p->run,
...
void start(struct PCB *pcb_p) {
  ...
  fprintf(
      fp,
      "At time %u\tprocess %u\tstarted arr %u\ttotal %u\tremain %u\twait %u\n",
      clk, pcb_p->data._id, pcb_p->data.arv, pcb_p->data.run, pcb_p->rem_t,
      clk - pcb_p->data.arv - pcb_p->exe_t);
  fprintf(mp,
          "At time %u\tallocated %u\tbytes for process %u\tfrom %u\tto %u\n",
          clk, pcb_p->data.mem, pcb_p->data._id, pcb_p->mem_s,
          pcb_p->mem_s + pcb_p->data.mem - 1);
}
...
void resume(struct PCB *pcb_p) {
  ...
  fprintf(
      fp,
      "At time %u\tprocess %u\tresumed arr %u\ttotal %u\tremain %u\twait %u\n",
      clk, pcb_p->data._id, pcb_p->data.arv, pcb_p->data.run, pcb_p->rem_t,
      clk - pcb_p->data.arv - pcb_p->exe_t);
}
...
void stop(struct PCB *pcb_p) {
  ...
  fprintf(
      fp,
      "At time %u\tprocess %u\tstopped arr %u\ttotal %u\tremain %u\twait %u\n",
      clk, pcb_p->data._id, pcb_p->data.arv, pcb_p->data.run, pcb_p->rem_t,
      clk - pcb_p->data.arv - pcb_p->exe_t);
}
...
void finish(struct PCB *pcb_p) {
  int clk = getClk();
  //(pcb_p->exe_t>1)?pcb_p->exe_t:1 this assures that not division be zero is
  //happening
  double WTA = (double)(clk - pcb_p->data.arv) /
               (double)((pcb_p->exe_t > 1) ? pcb_p->exe_t : 1);
  fprintf(
      fp,
      "At time %u\tprocess %u\tfinished arr %u\ttotal %u\tremain %u\twait %u\t",
      clk, pcb_p->data._id, pcb_p->data.arv, pcb_p->data.run, pcb_p->rem_t,
      clk - pcb_p->data.arv - pcb_p->exe_t);
  ...
  // This can be waitpid(no_pcb->pid, ...), but since this is a 1-core cpu, 1
  // process at a time, so no worries about which process it's gonna reap
  wait(&clk);
  *pcb_p = no_pcb;
  pcb_count--;
}
...
void FCFS() {
  ...
  // That is, if the previous if condition terminates a process, the next
  // process (which is being started down there) should start in the same second
...
void SRTN() {
  if (pcb[pcb_curr].state == RUNNING) {
    ...
    if (pcb[pcb_curr].rem_t == 0) {
      ...
      goto srtn_lbl;
      // you can add goto lbl; in here so not to recomputed the next_ function
      // again
    } else {
      unsigned pcb_past = pcb_curr;
      // No need to surroud this next_ function with a zero check, because it
      // either chooses the running process, or choose another that is waiting
      // or should be started what i mean is, that since there is a running
      // process (pcb_past), then this function will never return zero, you have
      // to understand
...
unsigned int BSA(unsigned int mem) {
  ...
  while (ff < MAX_MEM_SIZE) {
    if (sys_mem[ff] != -1) {
      ...
      space = 0;
      // Move ff by an amount that is equal to the largest of (sys_mem[ff] &
      // mem)
